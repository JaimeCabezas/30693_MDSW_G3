...
const normalizeUrl = (ruta) =>
  'http://localhost:8000/' + ruta.split('\\\\').join('/').split('\\').join('/');
  ...
  document.getElementById('input-traduccion').addEventListener('change', async (event) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/documentos/${documentoActivoParaTraduccion}/traduccion`,
        {
          method: 'POST',
          headers: { 'Authorization': 'Bearer ' + token },
          body: formData,
...
window.eliminarDocumento = async (id) => {
  ...
  try {
    const response = await fetch(`http://localhost:8000/documentos/${id}`, {
      method: 'DELETE',
      headers: { 'Authorization': 'Bearer ' + token },
    });
...
formCrearUsuario.addEventListener('submit', async (e) => {
  ...
  const url    = userId ? `http://localhost:8000/usuarios/${userId}` : 'http://localhost:8000/usuarios';
...
window.iniciarTraduccion = async (id, urlRaw) => {
  try {
    await fetch(`http://localhost:8000/documentos/${id}/iniciar`, {
      method: 'PUT',
      headers: { 'Authorization': 'Bearer ' + token },
    });
  } catch {
  ...
  window.open('http://localhost:8000/' + urlRaw, '_blank');
  await cargarDocumentos();
};
...
async function enviarEvaluacion(aprobado) {
  ...
  try {
    const response = await fetch(`http://localhost:8000/documentos/${id}/evaluar`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + token,
...
async function cargarEstadisticas() {
  try {
    const response = await fetch('http://localhost:8000/estadisticas', {
      headers: { 'Authorization': 'Bearer ' + token },
    });
    if (!response.ok) return;
...
window.generarBorradorIA = async (id) => {
  ...
  try {
    const response = await fetch(`http://localhost:8000/documentos/${id}/borrador-ia`, {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + token },
    });
...
async function cargarMensajes(id) {
  try {
    const response = await fetch(`http://localhost:8000/documentos/${id}/chat`, {
      headers: { 'Authorization': 'Bearer ' + token },
    });
    if (!response.ok) return;
